...
def save_display(fig, img_name, tab, x, y):
  fig.savefig('images/' + img_name, transparent=True)
  img = ImageTk.PhotoImage(Image.open('images/' + img_name))
  label = Label(tab, image = img)
  label.place(x, y)
  label.pack()

# summary bar chart 
db_fig = Figure(figsize=(5, 5), dpi=100)
report_ax = db_fig.add_subplot(111)
report_ax.bar(range(len(month_report[0])), month_report[1], align='edge', width=1.0)
db_fig.savefig('images/summary_bar.png', transparent=True)
summary_bar_img = ImageTk.PhotoImage(Image.open('images/summary_bar.png'))
label = Label(dashboard_tab, image = summary_bar_img)
